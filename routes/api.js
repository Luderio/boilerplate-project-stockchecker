...
module.exports = function (app) {
  ...
  app.route('/api/stock-prices').get(async function (req, res){
      const { stock, like } = req.query; //contains the response object from the forms. 
      ...
      if (Array.isArray(stock)) {
        ...
        const firstStock = await saveStock(stock[0], like, ipAddress);
        const secondStock = await saveStock(stock[1], like, ipAddress);
      ...
      const oneStockData = await saveStock(symbol, like, ipAddress);
